...
valid_batch = int(math.floor(valid_normal.shape[0] / batch_size) * batch_size)
test_batch = int(math.floor(test.shape[0] / batch_size) * batch_size)

df_c_normal = df_c[df_c['RATE_Apr16'] == 0]
df_c_anormal = df_c[df_c['RATE_Apr16'] == 1]

df_c_train = df_c_normal.loc[:split + 1]
df_c_valid = df_c_normal.loc[split + 1:]
df_c_train = df_c_train.loc[:train_batch]
df_c_valid = df_c_valid.loc[:df_c_valid.index[0] + valid_batch]
df_c_anormal = df_c_anormal.loc[:df_c_anormal.index[0] + test_batch]
df_c = pd.concat([df_c_train, df_c_valid, df_c_anormal], axis=0)

train_normal_x, train_normal_y = train_normal.values[:train_batch, 1:], train_normal.values[:train_batch, 1]
valid_normal_x, valid_normal_y = valid_normal.values[:valid_batch, 1:], valid_normal.values[:valid_batch, 1]
test_x, test_y = test.values[:test_batch, 1:], test.values[:test_batch, 1]
...
# MODEL
model = Sequential()
model.add(LSTM(100, batch_input_shape=(batch_size, train_normal_x.shape[1], train_normal_x.shape[2]), activation='tanh',
               stateful=True,
...
for i in range(5):
    model.fit(train_normal_x, train_normal_y, epochs=1, batch_size=batch_size, verbose=2, callbacks=[early_stopping],
              shuffle=False,
...
# PREDICT
# Train
train_predict = model.predict(train_normal_x, batch_size=batch_size)
model.reset_states()
...
# Valid
valid_predict = model.predict(valid_normal_x, batch_size=batch_size)
model.reset_states()
...
# PLOT
true_values = np.concatenate((train_true, valid_true, test_true), axis=0)
...
valid_true.index += len(train_predict.index)
test_true.index += len(train_predict.index) + len(valid_predict.index)

train_predict.index += 1
valid_predict.index += len(train_predict.index) + 1
test_predict.index +=  len(train_predict.index) + len(valid_predict.index) + 1

predict = pd.concat([train_predict, valid_predict, test_predict], axis=0)
...
df_c['DATE'] = pd.to_datetime(df_c['DATE'], format='%Y-%m-%d', errors='coerce')
print(df_c)
...
print('PASSENGER LOSS', test_true['true_values'].sum() - test_predict['predict'].sum())
